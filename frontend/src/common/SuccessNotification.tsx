...
type NotificationsOneProps = {
  debugNoRedirect: boolean;
...
export const SuccessNotification = ({
  ...
  visible,
  onClose,
}: NotificationsOneProps) => {
  // Auto-close the notification after 3 seconds if it is visible.
  useEffect(() => {
    if (!debugNoRedirect) {
      if (visible) {
  ...
  }, [visible, onClose, debugNoRedirect]);

  if (!visible) return null;

  const message =
    loginType === "admin"
      ? "Log in as Admin successful!"
      : "Log in as Guest successful!";

  return (
    <div className="max-w-full rounded-lg mt-6 py-4 pl-4 pr-4.5 shadow-2 dark:bg-meta-4">
